...
conf = args.config if args.config else "./config.toml"
...
for r in config["table"]:
    rconf = config["table"][r] # table row config
    table += '                    <th style="width: %s%%; text-align: left;">%s</th>\n' % (int(rconf["width"] * 100), rconf["header_text"])
...
for f in config["filters"]:
    fconf = config["filters"][f] # filter config
    if fconf["type"] == "fuzzy":
        fuse += '          const %sResults = %sQuery ? fuse%s.search(%sQuery).map(result => result.item) : data;\n' % (fconf["csv_col"], fconf["csv_col"], fconf["csv_col"], fconf["csv_col"])
