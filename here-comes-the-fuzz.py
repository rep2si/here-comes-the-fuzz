...
# Prepare filters for top of page
inputs_string = "<div>\n"
...
for f in config["filters"]:
    ...
    if fconf["type"] == "fuzzy":
        ...
        inputs_string += '    <input type="text" id="%s" placeholder="%s">\n' % (fconf["csv_col"], fconf["text"])
    elif fconf["type"] == "tick":
        if fconf["text"] != "":
            inputs_string += "    <span>%s </span>:\n" % (fconf["text"])
        for o in fconf["options"] :
            inputs_string += '    <label><input type="checkbox" id="%sCheckbox" value="%s">%s</label>\n' % (o, o, o)
